...
def create_token(data: str, key: bytes, mwt_id: UUID, purpose: bytes) -> str:
    ...
    segments = [
        ...
        base64.urlsafe_b64encode(purpose + b":" + mwt_id.bytes).rstrip(b'=')
    ]
...
def validate_token(token: str, key: bytes) -> tuple[str, UUID, bytes]:
    try:
        # segmentation
        data_b64, payload_b64, signature_b64 = token.split('.')
        payload = base64.urlsafe_b64decode(align_b64(payload_b64))
    ...
    if not validate_hmac(
        ...
        message=f"{data_b64}.{payload_b64}".encode(),
    ):
        raise InvalidSignature("Invalid signature")

    return data.decode("utf-8"), mwt_id, purpose
